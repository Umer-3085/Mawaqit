...
async def list_all_simple(service: TranslationTafseerDetailService = Depends(get_translation_tafseer_details_service)):
    return await service.get_all_simple()

@router.get("/{id}", response_model=TranslationTafseerDetailResponse)
async def get_detail(id: int, service: TranslationTafseerDetailService = Depends(get_translation_tafseer_details_service)):
