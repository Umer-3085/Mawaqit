...
async def get_verse_by_global(
    global_number: int, service: VerseService = Depends(get_verse_service)
):
...
async def get_verses_by_surah(
    surah_number: int = Path(..., ge=1, le=114), service: VerseService = Depends(get_verse_service)
):
...
async def get_verses_by_juz(
    juz: int = Path(..., ge=1, le=30), service: VerseService = Depends(get_verse_service)
):
...
async def get_verses_by_page(
    page_no: int = Path(..., ge=1), service: VerseService = Depends(get_verse_service)
):
