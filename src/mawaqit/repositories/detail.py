...
class TranslationTafseerDetailRepository:
    ...
    ) -> tuple[list[TranslationTafseerDetail], int]:
    ...
    async def get_all_simple(self) -> list[TranslationTafseerDetail]:
        result = await self.db.execute(
            select(TranslationTafseerDetail).order_by(TranslationTafseerDetail.id)
        )
        return list(result.scalars().all())

    async def get_by_lang(
        self, lang: str, page: int = 1, page_size: int = 100
    ) -> tuple[list[TranslationTafseerDetail], int]:
        return await self.get_all(page=page, page_size=page_size, lang=lang)

    async def get_by_direction(
        self, direction: str, page: int = 1, page_size: int = 100
    ) -> tuple[list[TranslationTafseerDetail], int]:
        return await self.get_all(page=page, page_size=page_size, direction=direction)

    async def get_by_author(
        self, author: str, page: int = 1, page_size: int = 100
    ) -> tuple[list[TranslationTafseerDetail], int]:
